$(function() {
   ...
   var tableFunctionaries = $('#table-parcelas-ajax').DataTable({
      ...
      columnDefs : [
         ...
         { targets : [4],
            render : function(data, type, row) {
               return `<button type="button" onclick="gerar_apenas_nfse('${row['cleannumber']}' ,'${row['idrelatorio']}', '${row['desnumero_parcela']}' )" class="btn ripple btn-primary w-100 font-weight-bold text-white" >APENAS NFSE</button>` 
              },  
         },
         { targets : [5],
            render : function(data, type, row) {
